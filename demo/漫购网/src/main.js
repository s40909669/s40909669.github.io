...
// Vue.use(VuePreview);
//v-viewer:引入v-viewer vue-preview更新了插件不太好用 此处用v-viewer
...
//引入全局组件需要的组件对象 开始
import NavBar from './components/common/navBar.vue';    //导航栏
import Comment from './components/common/comment.vue';  //评论
import MySwipe from './components/common/mySwipe.vue';  //轮播图
//引入全局组件需要的组件对象 结束
...
//         //VueRouter：配置路由规则
        ...
        { name:'news.list',path:'/news/list',component:NewsList}, //动漫新番
        { name:'news.detail',path:'/news/detail',component:NewsDetail}, //动漫详情
        { name:'photo.share',path:'/photo/share',component:PhotoShare}, //最新动漫热点
        { name:'photo.detail',path:'/photo/detail/:id',component:PhotoDetail},//热点详情
        { name:'goods.list',path:'/goods/list', component:GoodsList, mata:{
            // 后退不刷新
                keepAlive: true,
                isBack:false,
            }}, //热卖手办
        ...
        //商品介绍(新建一个路由拿动漫详情，改变path是为了拿不同于动漫详情的后台数据，但页面就用动漫详情的)
